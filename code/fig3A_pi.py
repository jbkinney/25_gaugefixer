...
if __name__ == "__main__":
    ...
    for axes, (p, position) in zip(subplots, enumerate(positions)):
        ...
        axes.yaxis.label.set_fontweight("bold")
        axes.yaxis.label.set_multialignment("center")
    ...
    # The "core" bracket and label are drawn above each axes (at y=1.15, with
    # clip_on=False). tight_layout reserves room for that overflowing artwork,
    # which shrinks every logo; setting the margins explicitly instead keeps the
    # logos tall and lets the brackets overlap the inter-panel gaps.
    fig.subplots_adjust(
        left=0.23, right=0.99, top=0.93, bottom=0.09, hspace=0.95
    )
    fig.savefig("results/fig3A_pi_raw.png", dpi=300, facecolor="white", edgecolor="none")
    fig.savefig("results/fig3A_pi_raw.pdf", facecolor="white", edgecolor="none")
    fig.savefig("results/fig3A_pi_raw.svg", facecolor="white", edgecolor="none")
