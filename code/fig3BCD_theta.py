...
plt.style.use("code/gaugefixer.mplstyle")
...
if __name__ == "__main__":
    ...
    fpath = "results/theta_fixed.aligned.csv"
    theta = pd.read_csv(fpath, index_col=0)
    theta0 = theta.loc[theta["k"] == 0, :].copy()
    
    fpath = "results/theta_fixed0.extended_core.csv"
    theta0_extended = pd.read_csv(fpath, index_col=0).drop(['orbit', 'subseq', 'k'], axis=1)
    theta0_extended = theta0_extended.set_index('label').T.reset_index()
    theta0_extended['index'] = theta0_extended['index'].astype(int)
    theta0_extended.sort_values(by='index', inplace=True)
    ...
    if plot_theta_0:
        ...
        for seq, color in zip(seqs, colors):
            ax.scatter(registers, theta0_extended[seq], c=color, s=3)
            ax.plot(registers, theta0_extended[seq], c=color, lw=0.5, linestyle="--")
        
        ax.scatter(registers, means, c="black", s=7, zorder=10)
        ax.plot(registers, means, c="black", lw=0.9, zorder=10)
        ...
        fig.savefig("results/fig3B_theta0_raw.png", dpi=300, facecolor="white", edgecolor="none")
        fig.savefig("results/fig3B_theta0_raw.pdf", facecolor="white", edgecolor="none")
        fig.savefig("results/fig3B_theta0_raw.svg", facecolor="white", edgecolor="none")
    ...
    if plot_theta_lc_lclc:
        ...
        for ax, (position, theta_lc) in zip(axes[:,0], theta_lc_positions.items()):
            ...
            # Extra pad so "register" reads as a subpanel title, not an axis label.
            # The label is rotated 90 deg, so the two lines stack side by side;
        ...
        # w_pad kept small so the additive heatmaps sit close to the pairwise
        # triangle plots; the "register" labels are spaced out via labelpad above.
        fig.tight_layout(h_pad=0.5, w_pad=1, pad=0.5)
        #plt.show()
        fig.savefig("results/fig3C_theta_raw.png", dpi=300, facecolor="white", edgecolor="none")
        fig.savefig("results/fig3C_theta_raw.pdf", facecolor="white", edgecolor="none")
        fig.savefig("results/fig3C_theta_raw.svg", facecolor="white", edgecolor="none")
    ...
    if plot_theta_scatter:
        ...
        #plt.show()
        fig.savefig("results/fig3D_theta_scatter_raw.png", dpi=300, facecolor="white", edgecolor="none")
        fig.savefig("results/fig3D_theta_scatter_raw.pdf", facecolor="white", edgecolor="none")
        fig.savefig("results/fig3D_theta_scatter_raw.svg", facecolor="white", edgecolor="none")
