...
\title{GaugeFixer: Removing unconstrained degrees of freedom in sequence-function relationships}
\author{[Author Names]}
\date{\today}
...
Linear models of sequence-function relationships are defined by a set of weights that multiply corresponding sequence features.
...
For example, a pairwise model for sequences of length $L$ built from $\alpha$ characters will have $\mathcal{O}(L\alpha)$ gauge freedoms. 
Gauge freedoms present a technical obstacle for researchers who want to interpret the quantitative parameters in their models. 
...
To project model parameters into a $\lambda$-$\pi$ gauge, one calls the method `fix'. The $\lambda$ parameter controls how much of the variance in model features is present in lower-order features vs. higher-order features. The $\pi$ parameter defines a factorizable probability distribution over sequence space. 
...
$\lambda = 1$, $\pi = \text{uniform}$ corresponds to the ``euclidean gauge''. This is the gauge into which parameters fall if they are inferred using standard L2 regression. 

$\lambda = \alpha$, $\pi = \text{uniform}$ corresponds to the ``equitable gauge''.

$\lambda = \infty$ corresponds to the ``hierarchical gauges''. Setting $\pi = \text{uniform}$ here yields the zero-sum gauge, in which model features summed over all characters at any one of the feature positions is zero. Setting $\pi$ to be an indicator function on a specific sequence yields the ``wild-type'' gauge. Here, parameters that correspond to features of order 1 and higher that appear in the wild-type sequence are zero, and the rest of the parameters describe mutations away from the wild-type sequence.

\section{Projection matrix vs algorithm gauge fixing}
Gauge fixing becomes complex 
