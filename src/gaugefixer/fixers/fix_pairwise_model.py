...
from gaugefixer.fixers._pairwise_series2dict import _pairwise_series2dict
from gaugefixer.fixers._pairwise_dict2series import _pairwise_dict2series
from gaugefixer.fixers._fix_pairwise_dict import _fix_pairwise_dict

@typechecked
def fix_pairwise_model(
    ...
    """Fix the gauge of parameters in a pairwise interaction model.
    
    This function applies gauge fixing to parameters in a pairwise interaction model.
    ...
    Raises:
        AssertionError: If invalid combination of gauge, pi_lc and wt_seq is provided,
            or if features don't match the expected pairwise features for sequence length L
    """
    alpha = len(alphabet)
    ...
    sorted_features = get_pairwise_features(L, alphabet)
    ...
    # Fix gauge
    theta_fixed_dict = _fix_pairwise_dict(theta_dict, p_lc=pi_lc)
