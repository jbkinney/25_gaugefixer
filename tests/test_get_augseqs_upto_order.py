...
def test_get_augseqs_upto_order_order_1_len_2():
    """Test up to order 1, sequence length 2, binary alphabet."""
    augseqs = get_augseqs_upto_order(2, 1, ['A', 'B'])
    expected = ['**', 'A*', 'B*', '*A', '*B']
    assert set(augseqs) == set(expected), f"Expected {expected}, got {augseqs}"

def test_get_augseqs_upto_order_order_1_len_3():
    """Test up to order 1, sequence length 3, binary alphabet."""
    augseqs = get_augseqs_upto_order(3, 1, ['A', 'B'])
    expected = ['***', 'A**', 'B**', '*A*', '*B*', '**A', '**B']
    assert set(augseqs) == set(expected), f"Expected {expected}, got {augseqs}"

def test_get_augseqs_upto_order_order_2_len_3():
    """Test up to order 2, sequence length 3, binary alphabet."""
    augseqs = get_augseqs_upto_order(3, 2, ['A', 'B'])
    # Order 0: ['***']
    # Order 1: ['A**', 'B**', '*A*', '*B*', '**A', '**B']
    # Order 2: ['AB*', 'BA*', 'AA*', 'BB*', 'A*A', 'B*A', 'A*B', 'B*B', '*AB', '*BB', '*AA', '*BA']
    # Total: 1 + 6 + 12 = 19 sequences
    assert len(augseqs) == 19, f"Expected 19 sequences, got {len(augseqs)}"

def test_get_augseqs_upto_order_by_order_sum():
    """Verify results against individual calls to get_augseqs_of_order."""
    # Test for sequence length 3, binary alphabet, up to order 2
    upto_seqs = get_augseqs_upto_order(3, 2, ['A', 'B'])
    
    # Get individual order sequences
    order0 = get_augseqs_of_order(3, 0, ['A', 'B'])
    order1 = get_augseqs_of_order(3, 1, ['A', 'B'])
    order2 = get_augseqs_of_order(3, 2, ['A', 'B'])
...
def test_get_augseqs_upto_order_count_check_ternary():
    ...
    augseqs = get_augseqs_upto_order(4, 2, ['A', 'B', 'C'])
    assert len(augseqs) == 67, f"Expected 67 sequences, got {len(augseqs)}"

def test_get_augseqs_upto_order_max_order_equals_seq_length():
    """Count check for maximum Order = sequence length."""
    # For length 3, binary alphabet, all possible augmented sequences
    augseqs = get_augseqs_upto_order(3, 3, ['A', 'B'])
    # Order 0: 1 sequence
